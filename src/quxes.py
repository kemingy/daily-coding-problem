...
# three colors: red, green, and blue. One power of the Qux is that if two of
# them are standing next to each other, they can transform into a single
...
def remain_quxes(quxes):
    for i in range(len(quxes) - 1):
        if quxes[i] != quxes[i+1]:
            another = MAP[(quxes[i], quxes[i+1])]
            return remain_quxes(quxes[:i] + [another] + quxes[i+2:])
    return quxes


if __name__ == "__main__":
    print(remain_quxes(['R', 'G', 'B', 'G', 'B']))
